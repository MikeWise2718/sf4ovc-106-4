...
- **Stripped upstream LFS-tracked files:** During the migration from `sf4ovc-106-4` to `sf4ov` (April 2026), we used `git filter-repo` to drop `readme-assets/` and `templates/extensions/` from history. These were 53 LFS-tracked files inherited from the upstream `kit-app-template` fork (banner images, per-template preview screenshots, sample USD stages, extension-template thumbnails). None are touched by `repo.bat build`, `repo.bat launch`, or sphereflake22 — they only matter if you run `repo template new` and pick one of those starter templates with sample data. Their LFS *blobs* were never actually published to GitHub LFS storage on `sf4ovc-106-4` either (only the pointer files were committed), so cloning the old repo also fails LFS hydration silently for these. We dropped them rather than carry over a broken-LFS state. If we ever need them back, they live in upstream NVIDIA-Omniverse/kit-app-template — restore selectively from there. The corresponding LFS-tracking lines in `.gitattributes` were also removed.
